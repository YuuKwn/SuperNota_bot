...
def get_results(update: Update, context: CallbackContext):
    ...
    if len(data['Search']) >= 4:
        ...
        update.message.reply_text(chat_id=update.effective_chat.id, text= 'Pick one', reply_markup=ReplyKeyboardMarkup(buttons, one_time_keyboard=True, selective=True))
